...
class GaussianModule(L.LightningModule):
    ...
    def connect_gui(self):
        ...
        while network_gui.conn != None:
            try:
                ...
                if custom_cam != None:
                    net_image = render(custom_cam, self.gaussians, self.config.pipe, self.background, scaling_modifer)["render"]
    ...
    def densify(self, viewspace_point_tensor, visibility_filter, radii):
        ...
        if self.global_step > self.config.train.densify_from and self.global_step % self.config.train.densify_interval == 0:
            ...
            self.gaussians.densify_and_prune(self.config.train.densify_grad_threshold, 0.005, self.scene.cameras_extent, size_threshold)
